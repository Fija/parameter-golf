#!/bin/bash
# Phase R — SP6144 CaseOps probe.
#
# Hypothesis (user observation): nobody in the entire PR queue has tried any
# vocab between 4096 and 8192. The leaderboard pattern 1024 -> 2048 -> 4096
# -> 8192 was a doubling sequence, not an optimum search. Phase L showed
# SP10240 hurts BPB and busts cap; SP6144 is the symmetric unexplored point.
#
# Stack: PR #1855's full 9-hparam stack on PR #1797 V2 base + BOS fix (in
# code) + EMBED_CLIP=14 (PR #1855's published value, NOT our Phase Q's
# relaxed 20). SP6144 has a smaller embedding table than SP8192 so we
# expect artifact comfortably under cap; no need for the EMBED_CLIP relax.
#
# Runs 3 seeds with abort guards: if seed-1 artifact > 16 MB or BPB > 1.062,
# stops immediately. Auto-stops pod via runpodctl + GraphQL on EXIT.
set -euo pipefail
set -x

BRANCH=${BRANCH:-submission/pr1797-ngram-mix}
SEEDS=${SEEDS:-"42 314 1234"}
RUN_ID_PREFIX=${RUN_ID_PREFIX:-phr}
...
cleanup() {
  ...
  echo "[$(date)] === Phase R done; auto-stop pod $POD_ID ==="
  if [ -n "$POD_ID" ]; then
...
# --- Pull SP6144 dataset (uploaded by Phase R0 prep) ---
DATA_DIR=/workspace/data/datasets/fineweb10B_sp6144_caseops
mkdir -p "$DATA_DIR"
SAMPLE=$DATA_DIR/datasets/datasets/fineweb10B_sp6144_lossless_caps_caseops_v1_reserved/fineweb_train_000000.bin
if [ ! -f "$SAMPLE" ]; then
  python3 - <<'PY'
import os
from huggingface_hub import snapshot_download
snapshot_download(
    repo_id="FijaEE/parameter-golf-sp6144-caseops",
    repo_type="dataset",
    local_dir="/workspace/data/datasets/fineweb10B_sp6144_caseops",
...
# --- PR #1797 V2 base + BOS fix + PR #1855 9-hparam stack + SP6144 ---
export DATA_PATH=$DATA_DIR/datasets/datasets/fineweb10B_sp6144_lossless_caps_caseops_v1_reserved/
export TOKENIZER_PATH=$SUB/tokenizers/fineweb_6144_bpe_lossless_caps_caseops_v1_reserved.model
export VOCAB_SIZE=6144 CASEOPS_ENABLED=1
...
# Full PR #1855 9-hparam stack — EMBED_CLIP back to 14 since SP6144 has small embed
...
PY
  ...
  if [ $SEED_NUM -eq 1 ] && [ -n "$ART" ]; then
    if [ "$ART" -gt 16000000 ]; then
      ABORT_REASON="seed-1 art $ART > 16M"; echo "[$(date)] === ABORT: $ABORT_REASON ==="; break
    fi
    if [ -n "$TTT" ]; then
