#!/bin/bash
# Phase S ROBUST — 4th attempt at SP10240 + 9-hparam stack.
#
# Diagnosis of prior failures:
#   - 3 community-cloud CPU pods died (pre-emption / mysterious stops with disk wipe)
#   - 1 secure-cloud H100 pod died after 64 min (script crash? GPU idle watchdog?)
#
# Robustness changes:
#   1. NO `set -e` -- log errors and continue / abort with diagnostics
#   2. Heartbeat log every 30s so we can see WHERE it died
#   3. GPU keepalive (1 small CUDA op every 30s) during retokenize phase
#   4. NO trap-on-EXIT pod stop; we stop pod ONLY at successful end
#   5. Per-seed CSV uploaded to HF immediately so partial results survive
#
set -u
set -x
...
# Heartbeat: run every 30s, write timestamped status with key process states.
heartbeat_loop() {
  while true; do
    {
      ...
      shards=$(ls /workspace/data/sp10240/datasets/*/fineweb_*.bin 2>/dev/null | wc -l)
      printf 'sp10240=%d\n' "$shards"
    } >> $HB_LOG 2>&1
    sleep 30
  done
...
# GPU keepalive: tiny CUDA op every 30s — defeats any "idle GPU" pod watchdog
gpu_keepalive() {
  python3 -u -c "
import time, torch
...
# Hard deadline kill — 6h
( sleep $((HARD_DEADLINE_MIN*60)) && echo "[$(date)] hard deadline" >> $HB_LOG && [ -n "$POD_ID" ] && runpodctl stop pod "$POD_ID" ) &
KILL_PID=$!

# Manual cleanup helper (called only on natural success)
final_cleanup() {
  echo "[$(date)] === Phase S done; auto-stop pod $POD_ID ==="
...
ls -la "$DOCS" || { echo "[FATAL] docs not found after download"; exit 1; }
...
if [ ! -f "$DATA_PATH_LOCAL/fineweb_train_000000.bin" ]; then
  ...
  python3 -u $SUB/prepare_caseops_data_parallel.py \
    ...
    --workers $WORKERS \
    --chunksize 128 || \
    { echo "[FATAL] retokenize crashed; ls of out dir:"; ls -la "$DATA_OUT/datasets/" 2>&1; exit 1; }
...
echo "[$(date)] === retokenize SP10240 DONE ==="

# Stop GPU keepalive before training (we want full GPU for training).
kill $KEEP_PID 2>/dev/null || true

# === Step 3: legality test ===
cd "$SUB"
python3 test_ngram_legality.py 2>&1 | tail -3 || echo "[WARN] legality test errored"
...
for SEED in $SEEDS; do
  ...
  # Upload progress CSV. If pod dies later, we still have this seed's result on HF.
