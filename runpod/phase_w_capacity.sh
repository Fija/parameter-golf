#!/bin/bash
# Phase W (Pod C) — More TTT capacity on PR #1953 stack.
# +3 levers vs PR #1953:
#   TTT_LORA_RANK 80 -> 96 (more per-doc fit capacity)
#   PHASED_TTT_NUM_PHASES 3 -> 4 (more global SGD updates of base_model on already-scored val)
#   PHASED_TTT_PREFIX_DOCS 2500 -> 2000 (rebalance — phase 1 shorter so 4 phases fit val)
# Hypothesis: more capacity in TTT layer captures more per-doc/inter-doc structure.
# Risk: artifact size grows (rank 96 > rank 80) — may cap-bust.
set -u
set -x
...
final_cleanup() {
  echo "[$(date)] === Phase W done; auto-stop pod $POD_ID ==="
...
# *** PHASE W NEW LEVERS: 3 capacity bumps ***
export TTT_LORA_RANK=96            # was 80
export PHASED_TTT_NUM_PHASES=4     # was 3
export PHASED_TTT_PREFIX_DOCS=2000 # was 2500 (rebalance — phase 1 shorter so 4 phases fit val budget)
export NGRAM_MIX_ENABLED=0 TEMP_SCALE_ENABLED=0 PPM_MIX_ENABLED=0

SEEDS=${SEEDS:-"42 314 1234"}
RUN_ID_PREFIX=${RUN_ID_PREFIX:-phw}
...
api.create_repo(repo_id="FijaEE/parameter-golf-phase-w-results", repo_type="dataset", private=True, exist_ok=True)
api.upload_file(
    path_or_fileobj=open(sys.argv[1], 'rb').read(),
    path_in_repo=f"results/{sys.argv[2]}_summary.csv",
    repo_id="FijaEE/parameter-golf-phase-w-results",
    repo_type="dataset", commit_message=f"{sys.argv[2]} progress",
)
PY

  if [ $SEED_NUM -eq 1 ] && [ -n "$ART" ]; then
    [ "$ART" -gt 16000000 ] && ABORT_REASON="seed-1 art $ART > 16M (cap bust from rank=96)" && echo "[$(date)] === ABORT: $ABORT_REASON ===" && break
    if [ -n "$TTT" ]; then
...
echo
echo "=== PHASE W FINAL (rank=96, phases=4, prefix=2000) ==="
cat $CSV
