#!/bin/bash
# Phase S — SP10240 + PR #1855 9-hparam stack + EMBED_CLIP=20.
#
# Hypothesis recap:
#   Phase L (April, an earlier config) ran SP10240 with MLP_CLIP=12,
#   EMBED_CLIP=15, TTT_LORA_RANK=96, default WARMDOWN_FRAC/BETA2 — i.e. NOT
#   the PR #1855 9-hparam stack. Result: pre-TTT -0.0015 BPB vs SP8192,
#   post-TTT +0.0017 BPB (worse), artifact 16.49 MB (over cap).
#
#   Phase Q proved EMBED_CLIP=14→20 saves ~280 KB on SP8192. Expected ~350 KB
#   on SP10240. PR #1855 9-hparam stack also tightens many other knobs and is
#   a strict superset of what Phase L tried.
#
# Phase S goals:
#   1. Cap fit: combine EMBED_CLIP=20 + LoRA=80 + MLP_CLIP=11.5 + 9-hparam
#      stack savings. Target ≤ 16 MB. Aborts seed-1 if > 16 MB.
#   2. BPB recover: TTT erased the -0.0015 BPB pre-TTT advantage in Phase L.
#      9-hparam tuning may handle the larger vocab better. Aborts if
#      seed-1 ttt_bpb > 1.062.
#   3. Record bar: would need mean ≤ 1.05914 (PR #1855 - 0.00194 BPB).
#      This is ambitious; the realistic best case is ~1.0598.
#
# Auto-stops pod via runpodctl + GraphQL on EXIT.
set -euo pipefail
set -x
...
# Full PR #1855 9-hparam stack + EMBED_CLIP=20 (Phase Q's cap-saver)
